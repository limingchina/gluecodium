...
public:
    ...
    std::shared_ptr<EnableTagsInDart> m_impl;
...
void register_smoke_EnableTagsInDart(py::module_& module) {
    py::class_<EnableTagsInDart, std::shared_ptr<EnableTagsInDart>, EnableTagsInDartTrampoline>(module, "smoke_EnableTagsInDart")
        .def(py::init<>())
        ...
        .def(py::init([](std::shared_ptr<EnableTagsInDart> native) {
            auto self = std::make_shared<EnableTagsInDartTrampoline>();
            self->m_impl = native;
            return self;
        }))
