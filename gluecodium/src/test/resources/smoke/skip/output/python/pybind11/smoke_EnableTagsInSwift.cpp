...
public:
    ...
    std::shared_ptr<EnableTagsInSwift> m_impl;
...
void register_smoke_EnableTagsInSwift(py::module_& module) {
    py::class_<EnableTagsInSwift, std::shared_ptr<EnableTagsInSwift>, EnableTagsInSwiftTrampoline>(module, "smoke_EnableTagsInSwift")
        .def(py::init<>())
        ...
        .def(py::init([](std::shared_ptr<EnableTagsInSwift> native) {
            auto self = std::make_shared<EnableTagsInSwiftTrampoline>();
            self->m_impl = native;
            return self;
        }))
