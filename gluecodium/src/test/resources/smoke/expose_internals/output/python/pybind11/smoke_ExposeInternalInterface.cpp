...
public:
    ...
    std::shared_ptr<ExposeInternalInterface> m_impl;
...
void register_ExposeInternalInterface(py::module_& module) {
    py::class_<ExposeInternalInterface, std::shared_ptr<ExposeInternalInterface>, ExposeInternalInterfaceTrampoline>(module, "ExposeInternalInterface")
        ...
        .def(py::init([](std::shared_ptr<ExposeInternalInterface> native) {
            auto self = std::make_shared<ExposeInternalInterfaceTrampoline>();
            self->m_impl = native;
            return self;
        }))
        ;
