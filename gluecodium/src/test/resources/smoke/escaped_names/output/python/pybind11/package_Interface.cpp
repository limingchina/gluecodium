...
void register_Interface(py::module_& module) {
    py::class_<Interface, std::shared_ptr<Interface>, InterfaceTrampoline>(module, "Interface")
        .def(py::init<>())
        ...
        .def(py::init([](std::shared_ptr<Interface> native) {
            auto self = std::make_shared<InterfaceTrampoline>();
            self->m_impl = native;
            return self;
        }))
        ;
