...
class OuterClassInnerInterfaceTrampoline : public InnerInterface {
public:
...
void register_smoke_OuterClassInnerInterface(py::module_& module) {
    py::class_<InnerInterface, std::shared_ptr<InnerInterface>, OuterClassInnerInterfaceTrampoline>(module, "smoke_OuterClassInnerInterface")
        ...
        .def(py::init([](std::shared_ptr<InnerInterface> native) {
            auto self = std::make_shared<OuterClassInnerInterfaceTrampoline>();
            self->m_impl = native;
            return self;
        }))
