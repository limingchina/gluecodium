...
using InternalListener = ::smoke::InternalListener;
...
public:
    ...
    std::shared_ptr<InternalListener> m_impl;
...
void register_smoke_InternalListener(py::module_& module) {
    py::class_<InternalListener, std::shared_ptr<InternalListener>, InternalListenerTrampoline>(module, "InternalListener")
        ...
        .def(py::init([](std::shared_ptr<InternalListener> native) {
            auto self = std::make_shared<InternalListenerTrampoline>();
            self->m_impl = native;
            return self;
        }))
        ;
