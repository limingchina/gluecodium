...
**Status**: In progress; initial Node.js functional coverage is passing
...
functional tests in the parallel `gluecodium1` checkout. It enables two feature groups for the
`js` generator:
...
The first tests found two embind generation defects that are fixed in this checkpoint:
