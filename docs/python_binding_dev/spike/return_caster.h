// Custom pybind11 type_caster for Gluecodium's Return<T, Error>.
//
// Design (matches plan §5.4 "Return<T, Error> failure -> Error subclass exception"):
//   - On success (has_value): the inner value is returned to Python automatically.
//   - On failure: a Python exception is raised instead of returning a value.
//
// The error string is derived via a small traits helper so the same caster works
// for both std::error_code and user-defined error types. In the real generator the
// caster would be specialized per error type to raise the matching py::exception<>.
#pragma once
