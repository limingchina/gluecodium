...
> **Date**: 2026-07-20 **Branch**: `python_bind` **Status**: Active follow-up; Phase A (A1-A7) and Phase C (C1-C4) complete; Phase D root cause re-diagnosed **Supersedes**: `phase8_followup_plan.md` (2026-07-18) — that document remains useful for Phase A/B/C history but its Phase D-K status no longer matches the codebase. **Related**: [phase8_status.md](./phase8_status.md), [python_pybind11_plan.md](../python_pybind11_plan.md)
>
...
| **G4**  | Method overloading — Python does not support overloaded method names                                                                        | 🟡 **Half-resolved.** pybind11-native side (`overload_cast`) is done. Python **wrapper class** generation (`PythonFunction.mustache` + `.pyi` stubs) still emits one clobbering `def` per overload — this is the real remaining work, see §2.4. | MethodOverloading, Properties, Defaults, **and, newly confirmed, Inheritance, MultipleInheritance, Errors, ExternalTypes**                                                |
...
4. **`.pyi` stubs get a different, better fix**: unlike the runtime `.py`, stub files support real `@typing.overload` stacking, which is strictly more useful to IDE/mypy users than a lossy `*args: Any`. `PythonStubFunction.mustache`/`PythonStubClass.mustache` should emit one `@overload`-decorated signature per Lime function, plus a final undecorated catch-all with a `...` body.
...
- `functional-tests/functional/python/test/method_overloads_test.py` already exists but only covers 5 of ~9 `isBoolean` overloads and none of `ConstructorOverloads`, `StructConstructorOverloads`, or `InheritanceOverloads.lime` — expand it to match. The existing assertions (`MethodOverloads.is_boolean(True)`, `.is_boolean(5)`, `.is_boolean("text")`, `.is_boolean(Point(...))`, `.is_boolean()`) are already written against the D0 dispatch-by-argument design, which is a good sign the target behavior was scoped correctly even before the wrapper bug was found.
...
| B         | 6        | Done               | —                                                     |
| C         | 4        | Done               | —                                                     |
| **D0**    | 1 (cross-cutting) | **1-2 days** | Medium — return-type-wrapping edge cases if a future feature introduces mixed-return-type overload groups |
