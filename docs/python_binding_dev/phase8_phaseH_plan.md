...
- **Python decision needed**: Python has no formal access-control keyword. Two options, consistent with the "internal" contract used elsewhere in Gluecodium (internal = library-internal API, not meant for external consumers, but callable from other internal Gluecodium-generated code / same package):
  ...
  **Decision (implemented 2026-08-04)**: Option 2 (leading-underscore convention) adopted. Rationale and implementation details:

  - `PythonNameRules.getName()` and `getPropertyName()` now prepend `_` to any element where `CommonGeneratorPredicates.isInternal(element, PYTHON)` returns `true`. A `@Python(Name=...)` override takes priority and bypasses the prefixing (so explicit renames are always honoured as-is).
  - `getFlattenedName()` (used for pybind11 C++ file names and `register_*` function names) also adds the underscore prefix for `@Internal` elements. This is necessary to avoid file name and register-function-name collisions when an internal type has the same LIME name as a public type (e.g. the `name_clash_overloads` smoke test, where `@Internal class AssetsManager {}` coexists with a public `class AssetsManager {}` in the same package — C++ avoids the collision via `@Cpp("AssetsManagerInternal")`).
  - The `pythonFilteredModel` no longer filters out `@Internal` elements; they are retained so the Python wrapper and pybind11 binding both emit them. The `isInternalOrNestedInternal` guard in `collectPybind11Types` and `generateCommonFiles` was removed, so internal types get pybind11 bindings and module-init `register_*` calls.
  - The `pybind11IncludeCollector` retainPredicate was changed from `isPublic` to `{ true }` so internal types' C++ headers are included.
  - A new `isInternalOrSkipped` predicate (combining `isSkippedForPython` and `isInternal`) was added to `PythonGeneratorPredicates.kt`. The `Pybind11StructInit.mustache` template now uses `isInternalOrSkipped` instead of `isSkippedForPython` to decide whether a field should be default-constructed in the lambda constructor (instead of being passed as a constructor parameter). Internal fields are excluded from constructor signatures (via the existing `isPublic` filter) but still present in the C++ constructor call (default-constructed).
  - `@Internal(Java, Kotlin)` (platform-scoped internal) correctly returns `false` for Python via `CommonGeneratorPredicates.isInternal(element, PYTHON)`, because the LIME parser stores platform-scoped `@Internal` as `JAVA.INTERNAL` + `KOTLIN.INTERNAL` attributes, not as the global `INTERNAL` attribute.
